...
class MLP_subgraph(nn.Module):
    def __init__(self, node_features_num, num_nodes, edge_features_num, device):
    ...
    def _edge_prob_mat(self, graph):
        ...
        f1 = x.unsqueeze(1).repeat(1, self.node_features_num, 1).view(-1, self.feature_size)
        f2 = x.unsqueeze(0).repeat(self.node_features_num, 1, 1).view(-1, self.feature_size)
